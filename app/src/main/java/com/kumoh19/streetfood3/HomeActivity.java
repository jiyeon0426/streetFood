...
import com.kumoh19.streetfood3.databinding.ActivityHomeBinding;
...
public class HomeActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        AppBarConfiguration appBarConfiguration = new AppBarConfiguration.Builder(
                R.id.navigation_home, R.id.navigation_category, R.id.navigation_ranking, R.id.navigation_add, R.id.navigation_mypage).build();
        ...
        strId = userInfo.get(0);
        strNickname = userInfo.get(1);
        strProfile = userInfo.get(2);

        //firebase 정의
        mDatabase = FirebaseDatabase.getInstance("https://streetfood-2c61e-default-rtdb.firebaseio.com/").getReference();
        readUser();

        //hashmap 만들기
        HashMap result = new HashMap<>();
        result.put("id", strId);
        result.put("name", strNickname);
        result.put("email", strProfile);

        writeNewUser(strId, strNickname, strProfile);


        // 주소 정보 데이터 받기
        Intent intent = getIntent();
        //address = intent.getStringExtra("address");
        Log.i("HomeActivity", "ADR : " + address);

//        address = ((Address)Address.context).ADR;
//        Log.i("HomeActivity", "ADR : " + address);
...
//        bundle.putString("name", strNickname); // 데이터 담기
//        //bundle.putString("profile",strProfile);
//
//        fragment.setArguments(bundle);

        //Log.d("HomeActivity","invoke: id " + strId);
        //Log.d("HomeActivity","homeseo: nickname" + strNickname);
        //Log.d("HomeActivity","homeseo: pro" + strProfile);

    }


    private void writeNewUser(String userId, String name, String image) {
        ...
        mDatabase.child("users").child(userId).setValue(user)
                .addOnSuccessListener(new OnSuccessListener<Void>() {
                    ...
                    public void onSuccess(Void aVoid) {
                        ...
                        //Toast.makeText(HomeActivity.this, "저장을 완료했습니다.", Toast.LENGTH_SHORT).show();
                    }
                })
                .addOnFailureListener(new OnFailureListener() {
                    ...
                    public void onFailure(@NonNull Exception e) {
                        // Write failed
                        //Toast.makeText(HomeActivity.this, "저장을 실패했습니다.", Toast.LENGTH_SHORT).show();
                    }
                });
    ...
    private void readUser(){
        mDatabase.child("users").child(strId).addValueEventListener(new ValueEventListener() {
            @Override
